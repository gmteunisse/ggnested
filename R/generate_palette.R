...
nested_colours <- function(df, group, subgroup, palette = NULL, base_clr = "#6495ed"){
  ...
  if (!group %in% colnames(df)){
    msg <- sprintf("Error: %s not in colnames(df)", group)
    stop(msg)
  }
  if (!subgroup %in% colnames(df)){
    msg <- sprintf("Error: %s not in colnames(df)", subgroup)
    stop(msg)
  }
  if (!is.data.frame(df)){
    msg <- sprintf("Error: df needs to be class 'data.frame', instead got '%s'", paste(class(df), collapse = ", "))
    stop(msg)
  }
